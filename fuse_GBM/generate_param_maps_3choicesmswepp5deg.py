...
f_donors = '/mnt/storage/home/pu17449/src/fuse_processing/GBM-5deg_distances_GBM-hisnowIQR_reducedset.pkl'
...
for choice in range(3):
	...
	f_out = os.path.join(griddir,'output/GBM-tiled2-2_'+str(dec)+'_MSWEPp5degcalibrated'+str(choice+1)+'.nc')
	...
	with Dataset(os.path.join(basedir,'fuse_grdc_4120800/output/grdc_4120800_'+str(dec)+'_mswep-p5deg_para_sce.nc'),'r') as f:
	...
	for catchment,indices in donor_indices.items():
		f_param = os.path.join(basedir,'fuse_grdc_'+str(catchment),'output','grdc_'+str(catchment)+'_'+str(dec)+'_mswep-p5deg_para_sce.nc')
