...
# for bp1:
griddir = '/work/pu17449/fuse/GBM-p1deg'
...
f_donor = '/home/pu17449/src/setup_scripts/fuse_GBM/GBM-p1deg_distances_GBM-reduced.pkl'
...
for choice in range(3):
	...
	f_out = os.path.join(griddir,'output/GBM-p1deg_'+str(dec)+'_MSWEP2-2-ERA5_calibrated'+str(choice+1)+'.nc')
	...
	with Dataset(os.path.join(basedir,'fuse_grdc_4120800/output/grdc_4120800_'+str(dec)+'_mswep-p1deg_para_sce.nc'),'r') as f:
	...
	for catchment,indices in donor_indices.items():
		f_param = os.path.join(basedir,'fuse_grdc_'+str(catchment),'output','grdc_'+str(catchment)+'_'+str(dec)+'_mswep-p1deg_para_sce.nc')
